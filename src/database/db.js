 ...
 * @param {string} userAddress
 */
IndexDB.prototype.getBlockedMediaByAddress = function(address, userAddress, callback) {
    this.select("SELECT m.*, " +
        "(SELECT count(*) FROM 'Like' l WHERE m.address = l.content_id) AS likes, " +
        "(SELECT count(*) FROM 'Following' b WHERE m.address = b.followed_address AND b.type = 17) AS blocks, " +
        "(SELECT count(*) FROM 'Following' b WHERE m.address = b.followed_address AND b.follower_address = '" + userAddress + "' AND b.type = 17) AS blocked, " +
        ...
        "(SELECT count(*) FROM 'Unlike' uld WHERE uld.author = '" + userAddress + "' AND uld.content_id = '" + address + "') AS user_unliked, " +
        "(SELECT count(*) FROM 'Media' m2 WHERE m2.author = a.address) AS publications FROM Author a) u ON " +
        "(u.user_address = m.author) WHERE m.address = '" + address + "' AND blocked >= 1 ORDER BY m.creation_date DESC;", callback)
};
 ...
 * @param callback
 */
IndexDB.prototype.getAllMediaByAuthor = function(authorAddress, userAddress, callback) {
...
IndexDB.prototype.removeBlock = function(unblock, callback) {
    this.run("DELETE FROM Following WHERE follower_address = '" + unblock.followerAddress + "' AND " +
        "followed_address = '" + unblock.followedAddress + "' AND type = 17;", callback);
};
